...
struct SegTree {
	...
	int sz = 1;
	vector<T> arr;
    T iden = {0ll, 0ll}; // define the identity element here
	...
	void set(int ind, pair<int,int> value){
	...
	void setUtil(int ind, pair<int,int> val, int x, int lx, int rx){
		if(rx - lx == 1){
...
void solve() {
    ...
    st.init(count + 1);
    for(int i = n - 1; i >= 0; i--){
    	auto [sum, num] = st.get(cord[a[i]] + 1, n);
    	int val = sum - num * a[i];
    	ans += val;

    	auto [cursum, curnum] = st.get(cord[a[i]], cord[a[i]] + 1);
    	st.set(cord[a[i]], make_pair(cursum + a[i], curnum + 1));
    }
